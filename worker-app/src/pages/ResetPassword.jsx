...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    async function validateToken() {
      ...
      try {
        const response = await api.get(`/api/workers/auth/reset-password/${token}`);
        if (!active) {
          return;
        }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await api.post(`/api/workers/auth/reset-password/${token}`, {
        password,
        confirmPassword
      });
      ...
      <section className="auth-section">
        <div className="auth-shell auth-grid">
          <div className="auth-copy">
            <p className="auth-copy-eyebrow">WORKER PASSWORD RESET</p>
            <h1>
              CREATE A NEW
              <span>WORKSPACE PASSWORD.</span>
            </h1>
            <p>Choose a new password for your Tasko worker account. This secure reset link can only be used once.</p>
